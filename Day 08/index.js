...
function solve(input) { // this part is the easy one xD, so there is some overhead in this function
...
function solve2(input) { //her were some nasty bugs, so I had to retry it again
    let digits = ["abcefg","cf", "acdeg","acdfg","bcdf","abdfg","abdefg","acf","abcdefg","abcdfg"];
    let result = 0;
    ...
    for(let i = 0; i < input.length; i++){
        ...
        let [given, toGuess] = singleLine.split('|').map(a=>a.trim().split(' ').map(a=>a.split('').sort().join('')));
